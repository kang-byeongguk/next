...
export async function GET() {
  try {
    // sql.begin()은 자동으로 트랜잭션을 시작하고, 
    // 내부 로직이 성공하면 COMMIT, 에러가 나면 ROLLBACK을 수행합니다.
    const result = await sql.begin(async (sql) => {
      ...
      // [2] Products 테이블 생성 (3NF: category_id FK 적용)
      await sql`
      ...
      // [3] 중복 제거된 카테고리 추출
      const uniqueCategories = Array.from(new Set(rawProducts.map(p => p.category)));
      ...
      // [5] 카테고리 ID 매핑 (이름 -> ID 찾기)
      // postgres 라이브러리는 결과값을 배열로 바로 반환합니다.
      ...
      const insertedProducts = await Promise.all(
        rawProducts.map(async (product) => {
          ...
          // postgres 라이브러리의 쿼리 문법
          return sql`
            INSERT INTO products (
              id, title, price, description, image, rating_rate, rating_count, category_id
            ) VALUES (
              ${product.id}, 
              ${product.title}, 
              ${product.price}, 
    ...
    return NextResponse.json({ message: 'Seeding Success (postgres-js)', data: result });
